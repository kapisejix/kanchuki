...
| 93 | **One app identity + one comment-stripper, both shared and both tested** — closes RC-041 properly and mints **RC-043** for the guard defect it exposed. **(1) Identity:** `packages/shared/src/constants/app-identity.ts` (new, via `@kanchuki/shared`) owns `APP_SCHEME` / `ANDROID_PACKAGE` / `IOS_BUNDLE_ID` + `appLink()` / `androidIntentUrl()`. Auditing RC-041's own fix found a **third** hand-written copy of the scheme — `apps/api/src/lib/staff-invite.ts` built `kanchuki://join?token=…` itself — so all three apps now read one module: the API through `appLink('join', { token })` (byte-identical output; a base64url token percent-encodes to itself), `apps/web/src/lib/deep-links.ts` reduced to a pure re-export holding **no literals**, and the mobile app a consumer rather than a fourth copy. `app-identity.test.ts` **11/11** pins the constants, asserts `apps/mobile/app.json` carries **no copy** of the three fields, that the dynamic config imports them and re-types no literal, and pins every `package`/`bundle` line in the two live operator docs (`docs/DEPLOY.md`, `docs/PLAY-STORE-RELEASES.md`) — searched: no other doc or marketing copy names the package, `play-store-listing.md` has none. **The manifest copy is gone (same day, later):** it had been kept because Expo resolves `app.json` before any TypeScript runs and moving the fields into the dynamic config touches the release path (`expo prebuild` → Gradle → the signed `.aab`). Done by importing shared's **source** — `require('sucrase/register/ts')` + the `@kanchuki/shared/constants/app-identity` subpath, whose `default` condition points at `src/` on purpose — because the built `dist/` is gitignored and the release path never builds it (measured: `Cannot find module …/dist/index.js`, *before* Gradle). `app.config.js` folded into **`app.config.ts`** (Expo prefers `.ts`, so a leftover `.js` beside it would be read by nobody — which would also have silently dropped the `extra.buildInfo` merge), and the resolved prebuild config was captured before and after and is identical — the comparison that also caught a transcription error in the edit (`READ_EXTERNAL_STORAGE` → `EXTERNAL_STORAGE` in `blockedPermissions`, the exact silent-regression mechanism row 72 documents). New `apps/mobile/__tests__/app-config.test.ts` **2/2** evaluates the **real** config through `@expo/config` — the only arm that can see a config that fails to load — and web's pin now compares `dist` against **source** (RC-035's trap, stated more precisely, now that no manifest holds the values). **(2) RC-043 — the guard that could not fail:** `stripComments` had been written from scratch in all **11** source-scanning guards (**6** on the first pass — **5** more were found by a re-grep of the regex *shape* before these docs were finalised, all five in the correct form, so no broken detector survived and "one implementation" was simply not yet true); **2** used the naive `//`-to-end-of-line form that deletes the `//` in `intent://` / `https://` — i.e. the exact text the arms search for, leaving them **vacuously green** (RC-041's own guard was one of the two) — and **2** more used `(^\|\s)`, which misses a comment after code with no space. One `packages/shared/src/testing/strip-comments.ts` (subpath `@kanchuki/shared/testing`, deliberately unable to reach an app bundle) with a **15/15** suite that reproduces the bug by name. All 11 migrated, so the regex exists in exactly one file — a checked fact rather than an assertion, since a grep of the shape outside the shared module now returns nothing. **Falsified 7 ways** — the decisive one twice in both directions (naive stripper + a hand-written `intent://` link on disk → web guard **10/10 green**; correct stripper + that same untouched link → **red, naming `lib/keyboard-inset.ts`**), plus dropping the `'line-first'` argument → red (the rake, on demand) and changing the helper's default order → red. Found on the way: the mobile guard's local helper carried a docblock *rationalising* the damage ("a truncated line only ever makes the guard quieter, never wrongly red") — quieter is the entire failure mode. The five late copies also showed the two passes **do not commute** — a line comment containing `/*` makes block-first open a block inside the comment and eat live code — so the order became an explicit `'line-first'` argument (two callers needed it), and that rake then fired for real on the first run after migration because `apps/api` reads the helper from the built **`dist/`**, which still held the one-argument signature (the RC-035 trap). Also found: the helper's own first documented limit was wrong (`label: // note` **is** stripped; the space precedes the `//`), caught by its suite. Suites: shared **62/62**, api **1432 passed / 5 skipped**, web **402/402**, mobile **125/125** (+2 for the config suite); `tsc` clean ×4 (the new `app.config.ts` is inside the mobile TS program, so a misspelled config field is now a type error), web `next lint` clean, api `biome check src/` clean (332 files). Falsified 6 ways on the manifest move (transpile hook removed → config cannot load; literal re-typed in the config; literal re-added to `app.json`; a field dropped from the merge; source changed without rebuilding `dist`, caught by **two** independent arms; and the docs pin re-checked after `app.config.ts` was added to its skip-list — the filename is skipped, the id on that line is not, verified red naming `docs/DEPLOY.md:1067`). No migration; no `apps/mobile` runtime module. | ✅ Built | 2026-09-25 | BUILD-LOG §2026-09-25 (RC-043), RC-041, RC-043 |
| 92 | **RC-041 — every web deep link is now built by one module pinned to the mobile manifest.** `apps/web/src/lib/deep-links.ts` owns `APP_SCHEME` / `ANDROID_PACKAGE` and the two builders (`appDeepLink()` → `kanchuki://…`, `androidIntentUrl()` → `intent://…#Intent;scheme=…;package=…;end`); all **four** hand-written call sites migrated — `join` (both links), `for-retailers` (the `?ref=` link) and `social/connect` (OAuth `code`/`state` callback, the params that can actually carry `&`/`=`/`#` and are now encoded). The bug: `/join`'s `intent://` carried `package=in.kanchuki.app`, the real package is `app.kanchuki.retailer`, so "Open with Android app" could never resolve to Kanchuki — Android fell through to default intent resolution, and it went unnoticed because the scheme-only `kanchuki://` link right above it still worked. `lib/__tests__/deep-links.test.ts` **10/10**: manifest pin, builder output, percent-encoding, absent params omitted, plus source scans letting no file but the module contain `intent://`, `kanchuki://`, a `package=` literal or the old wrong package. `join`'s test now asserts the full URI incl. `package=app.kanchuki.retailer` — it had pinned the `kanchuki://` link and **never** the `intent://` one, which is exactly why this shipped. **Falsified 2 ways, and the first found a defect in the guard itself:** the comment-stripper `.replace(/\/\/[^\n]*/g, '')` deleted `intent://` (it contains `//`), so both source-scan arms were **vacuously green** — every structural arm in the file could not fail. Fixed to only treat `//` as a comment when not preceded by `:`. **Third flawed-guard-found-by-mutation in two days** (RC-039's F7 resolved its own path from a literal filename); a guard's ability to fail has to be tested, since a green arm reads the same whether the rule holds or the detector is broken. Web **402/402** (50 files), `tsc --noEmit` + `next lint` clean. No migration, no `apps/mobile`/`apps/api` file. | ✅ Built | 2026-09-25 | BUILD-LOG §2026-09-25 (RC-041), RC-041 |
| 91 | **RC-042 — keyboard-inset shared primitives** (`<Sheet>` web + `KeyboardScreen` mobile), replacing the per-surface convention that produced the bug in the first place. Web: `apps/web/src/components/Sheet.tsx` (owns fixed overlay/scrim/panel, `paddingBottom` correction, `maxHeightVh` opt-in) over existing `lib/keyboard-inset.ts`; 11 surfaces migrated (AI Stylist, `CustomerConsentModal`, 9 admin files/11 dialogs) — 4 overlay shapes collapsed to one component, 9 inline `useKeyboardInset` sites deleted. Mobile: `apps/mobile/src/components/KeyboardScreen.tsx` (`KEYBOARD_BEHAVIOR = Platform.OS === 'ios' ? 'padding' : 'height'`, resolved once); 36 screens migrated, `KeyboardAvoidingView` now appears in exactly one file under `app/` (documented exemption: `staff/retailer-onboard.tsx`, iOS-only). Guards are **source-derived, not string-grepped**: `admin/__tests__/keyboard-inset.test.ts` (21 arms) derives every admin overlay holding a text field and requires adopt-or-exclude-with-reason; `__tests__/keyboard-screen-adoption.test.ts` (8 arms) bans the raw RN view and the re-derived iOS/Android ternary from `app/`. 10 falsifications, all red for the right reason, restored. Web **392/392** (49 files), tsc + `next lint` clean; mobile **123/123** (20 files), tsc + eslint clean. **Not verified: real-device behavior on either platform** — no desktop browser can exercise the web path (`visualViewport.height === window.innerHeight` with no keyboard, so the inset stays 0 in Chromium/desktop WebKit — a green desktop run is a false green); Android is a different mechanism (`height` reflow) needing its own device. Both are §7B.9 procedures, not claims. | ✅ Built | 2026-09-25 | BUILD-LOG §2026-09-25 (same day, later), RC-042, board §7B.9 |
...
|----|----------------------|----------|
| RC-043 | **Eleven copies of one test helper, two of them broken, and nothing tested the helper — so a guard that could not fail read exactly like a guard that passed.** `stripComments` (or an inline `code()`) was written from scratch in all eleven source-scanning guards (**six** found on the first pass, **five** more by a re-grep of the regex *shape* — all five in the correct form, so nothing broken survived). Two used `.replace(/\/\/[^\n]*/g, '')`, which treats **every** `//` as a comment opener — including the `//` in `intent://`, `https://` and `kanchuki://` — so on any line mentioning one of those it deleted the rest of the line, i.e. the exact text the surrounding arms search for, leaving them **vacuously green** (RC-041's own guard was one of the two). Two more used `(^\|\s)\/\/`, which misses a comment that follows code with no space (`x();// idm-vton`), so a banned string written there survived the strip and failed later with a confusing message. Invisible because a broken stripper only ever *removes* text: "found nothing" is what a working guard says about a clean tree **and** what a vacuous one says about a dirty one — the same string, so no assertion could tell them apart without a mutation. Same family as RC-041/RC-034 (a rule written out per call site), with a worse consequence: the duplicated value was the correctness of the guard itself. | `packages/shared/src/testing/strip-comments.ts` — one implementation, exposed as the `@kanchuki/shared/testing` subpath so it cannot reach an app bundle (`exports` maps only `./testing`; no runtime module imports it), with each clause of the regex justified in place. All 11 migrated; the regex exists in exactly one file — a checked fact now, because grepping the shape outside `packages/shared/src/testing/` returns nothing. The five late copies also showed the two passes **do not commute** (a line comment containing `/*` makes block-first open a block inside the comment and eat live code), so the order became an explicit `'line-first'` argument — and the rake fired for real on the first run, because `apps/api` reads the helper from the built `dist/`, which still held the one-argument signature (the RC-035 trap). `(^\|[^:])` keeps a `//` after a `:` intact (the scheme case) and `$1` returns the seam character (dropping it would join the cut and could invent or hide a token). Suite **15/15** including `keeps a real link when a commented-out one sits above it` and the order arms. **Falsified in both directions with the same violation untouched:** naive stripper + a hand-written `intent://` link in `apps/web/src/lib/keyboard-inset.ts` → the web guard passed **10/10**; correct stripper + that same link → `expected [ 'lib/keyboard-inset.ts' ] to deeply equal []`. Also corrected: the helper's first documented limit was wrong (`label: // note` **is** stripped — the space precedes the `//`); only a `//` immediately after `:` is exempt, and that distinction now has its own assertion. See row 93. |
| RC-042 | **The keyboard inset was a platform behaviour every overlay/screen had to apply and none did, because it lived as a convention (copy-pasted `useKeyboardInset` calls / `Platform.OS === 'ios' ? 'padding' : 'height'` ternaries) instead of as a component with an enforced adoption list.** On iOS, focusing an input inside a bottom-anchored sheet put the input behind the keyboard with no way to scroll to it. Nothing logs, nothing throws, no test goes red — the layout is valid, just covered. Compounded by **no desktop browser being able to show it missing**: `visualViewport.height === window.innerHeight` with no software keyboard, so the computed inset is always `0` in Chromium and desktop WebKit, including device emulation — a green desktop run is a false green. | this session — `apps/web/src/components/Sheet.tsx` (+ existing `lib/keyboard-inset.ts`) and `apps/mobile/src/components/KeyboardScreen.tsx` are now the sole owners; both guards **derive** their adoption set from source instead of grepping a magic string, so a new overlay/screen with a text field must be adopted or excluded with a reason. 10 falsifications restored. Web 392/392, mobile 123/123. Real-device verification (iOS + the Android `height` arm) still open — §7B.9. See row 91 |
| RC-041 | **A package name hand-duplicated across two files with no shared source and nothing pinning them together.** `apps/web/src/app/join/page.tsx`'s `intent://` deep link hardcodes `package=in.kanchuki.app`; the real Android package is `app.kanchuki.retailer` (`apps/mobile/app.json`). "Open with Android app" on the staff-invite bridge therefore cannot resolve to Kanchuki — Android falls through to default intent resolution instead. Invisible because the plain `kanchuki://` scheme link right above it (no package to get wrong) still works, so casual testing hits the working path. Surfaced while cross-checking the package name for §5B.1's new deep link (`for-retailers/page.tsx`), which does not copy the error forward; recorded found-not-fixed, then fixed in the same session. | The literals now live in **`packages/shared/src/constants/app-identity.ts`** (`APP_SCHEME` / `ANDROID_PACKAGE` / `IOS_BUNDLE_ID` + `appLink()` / `androidIntentUrl()`) — see **row 93** and **RC-043**, which found a third hand-written copy (`apps/api/src/lib/staff-invite.ts`) while auditing this fix. `apps/web/src/lib/deep-links.ts` became the only web source first (all four call sites migrated: `join` ×2, `for-retailers`, `social/connect`), then a pure re-export holding no literals; `lib/__tests__/deep-links.test.ts` (**10/10**) pins web's dist-resolved constants to shared's **source** — the RC-035 stale-`dist` trap — and scans `src/` so no web file may contain `intent://`, `kanchuki://`, a `package=` literal, the package id or the old wrong package. `join`'s own test now asserts the full URI including `package=app.kanchuki.retailer` — it had pinned only the `kanchuki://` link, which is why this shipped. Falsified 2 ways, and one found a defect **in the guard**: the comment-stripper ate `intent://` (it contains `//`), leaving both source-scan arms vacuously green. See row 92. |
...
| RC-037 | **The mocks returned the same object they stored, so three payout bugs were invisible — the test double agreed with the code, not with the platform.** (1) RC-036's partial-claim resize mutated the JS object `tx.referralPayout.create()` returned, and the mock's `create` pushed and returned the *same* object, so the mutation leaked into "the DB" and the test passed — while `submitPayoutRow` re-reads the row, i.e. RazorpayX would have been paid the stale figure. (2) `request.rawBody` is set by a `preParsing` hook registered inside `billingRoutes`; Fastify hooks are plugin-scoped and the payout webhook is a separate plugin, so it was always undefined — **every** RazorpayX webhook would 401, and the source-scan tests checked the secret *name*, never that the body was captured. (3) `schema.prisma` declares enum `referral_payout_account_type` (BANK_ACCOUNT/VPA) while migration 113 made the column TEXT with a lowercase CHECK and never created the type — mocked Prisma never casts, so every payout-account save would fail; 113 also stored raw bank/UPI details the owner decision said must never persist, and nothing read them. Two logic defects found in the same review: the claim attached only `payout_id IS NULL` conversions, but a PAID batch keeps its conversions attached while they keep accruing, so after a referrer's **first** payout nothing ever re-attached and every later month hit `EmptyClaimError`; and any submit exception released the claim, re-batching the money under a **new** idempotency key — a double-pay whenever RazorpayX had in fact created the payout. | `d585d26b` (`fix(referrals): five T7 payout defects hidden by mocks`) — the claim tx takes `pg_advisory_xact_lock(hashtext(referrer_id))`, re-reads the ledger under it, creates the batch at that amount (no post-hoc resize), and attaches conversions unattached **or** held by a PAID batch; `RazorpayxHttpError.isDefinitiveRejection` (4xx except 408/429) is the only thing that releases a claim; one shared `captureRawBody` hook replaces three copies (billing, WhatsApp, RazorpayX); migration `115_referral_payout_account_fix` (**owner must apply**) converts the column to the enum and drops `bank_details`/`vpa_address`. Payout job **53/53**; falsified both ways (forcing every error "definitive" → the ambiguous-5xx test red; sizing from the pre-read → partial-race + F1 red). Full API 1327/1332 (5 skips), tsc clean |
