...
**2026-08-04 device-test findings** — full detail `docs/PROGRESS.md` "SecureStore crash fix + open bug". SecureStore crash fixed (`theme.tsx` cache key had a `:`, only `[A-Za-z0-9._-]` allowed). Open: AI-tagged name/subtype/SKU/description still blank on `product/[id].tsx` after tagging — code path checked clean end-to-end, prime suspect is the prod `AI_TAGGING` worker not consuming (device points at `https://api.kanchuki.app`), not a client bug. Also requested: color-detect circle on `product/add.tsx` at capture time (exists on edit screen only, not ported to add flow yet).
