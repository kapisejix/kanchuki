...
### 🚩 Session handoff (2026-08-12 EOD — a new session should read this first)

**Git state:** feature work committed + pushed (`1462153` = the MSG91 OTP feature + webhook receiver + user-exists endpoint + Dockerfile `||` retry). **UNCOMMITTED in the working tree:** (1) the onnxruntime build fix below, (2) this handoff block. Deploy status: the `1462153` API build **FAILED on Railway** — onnxruntime CUDA download from GitHub (SEA-region build egress flake); the live API is still the 2026-08-11 image. ⚠️ **The old build 401s EVERY unknown path** (catch-all returns `Missing Bearer token`) — that was a red herring while debugging; the new MSG91 routes are correct (verified via a local Fastify 5.10 repro of the `/v1/public` auth exemption in `plugins/auth.ts`).

**PENDING — user is handling the push:** `ENV npm_config_onnxruntime_node_install_cuda=skip` in **both** `apps/api/Dockerfile` and `apps/web/Dockerfile`. onnxruntime-node's postinstall only downloads the CUDA EP providers (~330MB) from GitHub — the base ONNX Runtime binary ships in the npm package and production containers have no GPU, so skipping is safe (verified: env → `skip` → `NO_INSTALL` → `process.exit(0)` before any fetch). The `||` retry stays as cheap insurance. **The dashboard Redeploy button re-runs the OLD snapshot — a push is required** for the fix to reach the build.

**Next-session checklist:**
1. Confirm the API deploy goes green (build log: onnxruntime postinstall completes with NO `Downloading…` line).
2. Verify endpoints live: `GET https://api.kanchuki.app/v1/public/msg91/user-exists?identifier=917000000000` → `{user_found:true,…}`; `POST /v1/public/webhooks/msg91/events` with `x-msg91-webhook-secret` → `{received:true}`. No header = **503** if `MSG91_WEBHOOK_SECRET` is unset on the API env, **401** if set — that distinction is the config check.
3. Confirm API env: `MSG91_AUTHKEY` + `MSG91_TEMPLATE_ID` (user said values were added — verify).
4. MSG91 dashboard: save webhook URL `https://api.kanchuki.app/v1/public/webhooks/msg91/events` + header `x-msg91-webhook-secret`; **User Existence field: leave blank** (or the user-exists URL above if the dashboard forces it).
5. Mobile: **EAS build required** (native module — Expo Go can't run it); test send/verify/invisible-mode on a real phone; web widget needs a deploy with the `NEXT_PUBLIC_MSG91_*` build args.
6. Lock the verifyAccessToken response shape: `npx tsx scripts/verify-msg91-token.ts "<widget_jwt>"` with a real JWT from a widget login.
