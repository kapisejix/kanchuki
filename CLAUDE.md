...
| 55 | Onboarding Plan Selection — mandatory step 4 after GST, before "Done"; Demo (full Pro, no payment) + Starter/Growth/Pro cards; `demo_plan: true` grants PRO/TRIAL limits | ✅ Built | 2026-08-31 | BUILD-LOG §55 |
| 56 | Admin bodyless-POST 400 fix — unsuspend/feature/unfeature 400'd on Fastify v5 empty JSON body (`adminMutateOptions()` always sends `Content-Type: application/json`); tolerant `parseJsonAllowEmpty` parser + red error banner + real API error surfaced | ✅ Fixed | 2026-08-31 | BUILD-LOG §56 |
