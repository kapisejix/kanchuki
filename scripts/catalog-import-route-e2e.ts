...
async function main() {
  ...
  const up = await api<{ data: { upload_url: string; public_url: string } }>('/v1/catalog-import/upload-url', {
    method: 'POST',
    auth: true,
    body: { filename: DEMO_IMAGE, content_type: contentType, size_bytes: file.length },
  })
  const putRes = await fetch(up.data.upload_url, { method: 'PUT', headers: { 'Content-Type': contentType }, body: file })
  ...
  const detect = await api<{ data: { total_items: number; items: Array<Record<string, unknown>> } }>(
    '/v1/catalog-import/detect-items',
    { method: 'POST', auth: true, body: { image_url: up.data.public_url } },
  )
  ...
  for (const item of detect.data.items) {
    ...
    console.log(`  - ${item['description']} | category=${tags['category']} color=${tags['primary_color']} neck_style=${JSON.stringify(tags['neck_style'])}`)
  }
  ...
  console.log(`  created ${bulk.data.total_created} product(s):`, bulk.data.products.map((p) => p.id).join(', '))

  console.log('\n[5/5] verify products retrievable via /v1/products/:id')
  for (const p of bulk.data.products) {
    const got = await api<{ data: { id: string; category: string | null } }>(`/v1/products/${p.id}`, { auth: true })
