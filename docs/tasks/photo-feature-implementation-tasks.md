...
- [x] Add fast/slow quality mode toggle for retailers
- [x] Pre-warm Python pipeline to reduce cold-start latency
- [ ] Improve local dev fallback error handling (currently just a warning)

### Virtual Try-On
- [ ] Add GPU detection on V-Tone box with auto-fallback
- [x] Implement exponential backoff polling (2s → 4s → 8s → 16s max)
- [x] Add 24-hour expiration countdown timer in TryOnModal UI

### AI Studio Shoots (F-032 Phase A)
- [x] Optimize polling with adaptive backoff: 1s × 10 polls, then 3s → 5s → 10s → 15s
- [x] Add progress percentage and ETA to Redis job status
...
### Product Gallery
- [x] Add lazy loading for non-priority images using IntersectionObserver
- [x] Add disabled/visual state for SOLD color chips
...
- [x] AI Studio Shoots: Growth/Pro only (STARTER → 402)
- [x] Virtual Try-On: Monthly quota (STARTER=0, GROWTH=100, PRO=500)
- [x] Photo Cleanup: Already sidecar-separated, no plan gate needed
