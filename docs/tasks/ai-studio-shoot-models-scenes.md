...
**Status:** Steps 1–5 BUILT (unmerged, admin-bench only) 2026-08-30 — see BUILD-LOG §2026-08-30 (demographic). **Step 6 SUPERSEDED** by the DB-backed style catalog spec `docs/superpowers/specs/2026-08-30-studio-styles-admin-design.md` (📋 planned, awaiting owner go) — that work replaces `STUDIO_TEMPLATES`/`STUDIO_MODELS` with a `studio_styles` table + admin manager + per-plan assignment, and folds in the mobile Product/Models tab split + demographic auto-filter that step 6 described.
**Created:** 2026-08-30
