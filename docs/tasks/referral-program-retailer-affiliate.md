...
| T6 — Commission calc + ledger | ✅ Built 2026-09-23 | `jobs/referral-accrue.ts` + cron `15 2 * * *` + migration `112_referral_accrual_columns` (**not applied**) |
| T7 — Payout job + webhook + payout accounts | ✅ Built 2026-09-23 | `jobs/referral-payout.ts` (cron `30 2 30 * *`) + `lib/razorpayx.ts` + `lib/referral-payout-settle.ts` + `routes/webhooks/razorpayx-payout.ts` + `routes/retailers/retailers-payout-account.ts` + migrations `113`/`114` (**not applied**). Pays nobody until RazorpayX is live + 109–114 applied |
| T8 — Retailer "Refer & Earn" screen | 🔴 Not started | **Wants `apps/mobile` — blocked on Play Console review; ask the owner first** |
| T9 — Admin monitoring | 🔴 Not started | Interim payout-account entry path for retailers until T8 ships |
| T10 — Final tests + docs | 🔴 Not started | §11 checklist run at the end |
...
- Migrations **113** (`referral_payout_accounts`) and **114** (tax columns) created; **not applied** (admin dashboard, with 109–112).
