...
| CatVTON-on-RunPod try-on, admin-gated launch + dual quota (F-039 Phase 2) | `pending/catvton-runpod-tryon-launch.md` | 🟨 Built, flag OFF — build now, launch later (owner go-ahead 2026-09-26) | T0 ✅, T1 ✅, T2 ✅, T3 ✅ 2026-09-26 — endpoint + image alive; schema + customer quota built (migrations 118/119 **proposed, not applied**); route + job built (server-side 404 gate, dual identity via Bearer or passport cookie, wearer photo handed over in-process so it never reaches R2/DB/**Redis**). **Before it works live:** apply migrations 118→119, rebuild the GHCR worker image, `workersMax > 0` (owner's RunPod key). T4–T6 (UI, admin customer-limit field, consent/notice text) still to build before the flag flips on |
