...
**Status:** 🟡 Design only — retail-side/billing engine not built; awaiting decisions (see §8). **Partly built 2026-09-03:** the admin-side **addon-pack management** half is live — migration `089_resource_packs` applied in prod, CRUD API + `/admin/resource-packs` screen (DB-driven, no hardcoded values) — see `CLAUDE.md` row 60. What remains is the retailer-facing consumption of those packs (per-API credit limits, free allowance per plan, top-up purchase flow, per-retailer usage metering). Re-checked against code 2026-09-23 during the docs reorg; nothing else from this spec has landed.
**Date:** 2026-08-28
