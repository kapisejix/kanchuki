...
- [x] **5B.1** Carry the code into the app — **done 2026-09-25, scope reduced by a real blocker.** The board's literal ask (`/r/<CODE>` page + Play Store button with Install Referrer) assumes a live Play Store listing — **there isn't one.** Grepped the whole repo: zero `play.google.com` URLs anywhere, and `for-retailers/page.tsx`'s own copy says so — *"Play Store / iOS app listings — Coming soon, Android APK available now."* Install Referrer is a Play-Console-only mechanism; wiring it against a listing that doesn't exist is the RC-025 shape (described in one place, implemented in none), so it was **not** built. Confirmed with the owner before proceeding — see the three-option decision recorded in session. **What shipped instead:** `?ref=<CODE>` capture on the link T3 **already** mints (`buildReferralLink()` → `/for-retailers?ref=<CODE>`) — reused rather than inventing a new `/r/<CODE>` route, so there is one referral-link shape, not two. A valid-shaped code (cosmetic display gate only, `KAN-XXXXXX`; T4's server-side write is the real validator) renders a banner with the code + a `kanchuki://onboarding?ref=<CODE>` deep link for retailers who already have the app installed. Guard `apps/web/src/app/for-retailers/__tests__/page.test.tsx` **5/5**: valid code renders; absent/garbage/wrong-shape (staff F-018 shape, hyphen-stripped affiliate shape) all render nothing. Falsified: widened `REF_CODE_PATTERN` to `/^.*$/` → the 3 negative arms went red for the right reason, restored. **While cross-checking the deep-link package name against `apps/mobile/app.json`, found and recorded (not fixed) RC-041**: the existing `/join` bridge page hardcodes the wrong Android package in its `intent://` URI (`in.kanchuki.app` vs the real `app.kanchuki.retailer`) — pre-existing bug, out of scope here, the new code does not copy it forward. Web **336/336**, tsc clean.
...
| Task | Commit | Date |
|---|---|---|
| 5B.1 — `?ref=<CODE>` referral capture on `/for-retailers` (reused T3's existing link shape, no new route) + `kanchuki://onboarding?ref=` deep link; guard 5/5, falsified; found + recorded RC-041 (wrong Android package in `/join`'s `intent://` link, not fixed — out of scope) | *(this commit)* | 2026-09-25 |
| 7A.5 — k6 load-test scripts (`storefront.js` anonymous read-heavy, `retailer.js` authenticated) + guard (`load-test.test.ts` 54/54, falsified 4 ways) + runbook (`docs/SCALING.md (Load Testing section)`); §7A now complete | *(this commit)* | 2026-09-24 |
...
| 7A.4 — disaster-recovery runbook (`docs/SECURITY.md (Disaster Recovery Runbook section)`) | *(this commit)* | 2026-09-24 |
