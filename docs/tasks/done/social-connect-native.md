...
**Status:** ✅ **Code shipped — done.** One residual, tracked elsewhere: it needs an EAS build + Meta App Dashboard config (redirect URI, App Mode/App Review) before it works on a real phone, so the real-device verification is carried as an open item in `docs/tasks/pending/launch-readiness.md` (not a gap in this doc).
...
> → **Full Meta-dashboard runbook: [`docs/references/guides/meta-facebook-login-setup.md`](../../references/guides/meta-facebook-login-setup.md)**
