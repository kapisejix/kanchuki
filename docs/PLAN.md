...
### F-036 Customer PWA — Home-Screen Icon, Visited-Store List & Push Notifications — 🔴 Planned
Spec `docs/tasks/customer-pwa-store-list-and-push-notifications.md`, full requirement `docs/PRO-REQUIREMENTS.md` §32. Builds on the Shopper Passport identity work (`docs/customer/customer-qr-identity-solution.md`; `CustomerAccount`/`CustomerStoreVisit` already in schema via migrations `079`–`081`). Gives customers an installable PWA icon that opens a `/my-stores` list of every retailer they've visited (tap-through to the existing catalog, no change there), plus Web Push notifications on new-product upload and new-collection publish — a free, additive channel wired to the existing product-create/collection-publish events. **WhatsApp share is untouched** — push does not replace it, it only covers routine updates retailers wouldn't otherwise send manually. Net-new: `PushSubscription` model + RLS, subscribe/unsubscribe API, VAPID + `web-push` send job, a `push` handler added to the currently precache-only service worker (`apps/web/public/sw.js`), `/my-stores` page, Android install-prompt CTA, and an enforced iOS "Add to Home Screen" step (Safari 16.4+ requires the PWA be installed before it can receive push at all — a plain tab cannot). Roadmap: Phase A (store list + icon) → B (push, Android-first) → C (iOS parity) → D (consent/mute UI + retailer-side subscriber counts). No dependency on Phase 1 AI Core; can ship any time after the passport identity pieces are in place.
