...
### Future: F-024 DB-Backed Default Shop-By Categories + AI Auto-Category Assignment — 🔴 Reviewed 2026-08-04, NOT STARTED
Move the "Shop By Categories" default list (15 garment-type categories: Kurta Sets, Salwar Suits, Short Kurtis, Kurta, Co-ords, Plus Sizes, Dresses, Bottoms, Lehengas, Loungewear, Sarees, Shirts for Women, Tops for Women, plus New Arrivals + Sale) from hardcoded to DB, seeded per-retailer, admin-editable template — and have AI tagging auto-assign `Product.category_id` by matching the tag result against the retailer's own category list, so retailers stop picking categories by hand. Reuses the existing `ProductCategory`/`categories.ts` CRUD wholesale — no parallel category system. New Arrivals/Sale flagged as date/price-derived, not photo-taggable — recommend computing them as virtual query-time filters (option A) rather than AI-assigned rows (option B). Full design + open decisions: `docs/PRO-REQUIREMENTS.md` §14. **Do not start coding without explicit go-ahead.**

### Future: F-025 Scan-to-Sell — Offline Sale Reconciliation via SKU/QR Scan — 🔴 Reviewed 2026-08-04, design approved, NOT STARTED
Retailer sells an item in the physical shop; nothing updates the digital catalog today (`Product.status` + manual toggle already exist, already offline-safe via the mutation queue, but the trigger requires opening the app and finding the product manually). Decided design: **scan folded into the existing product-detail screen**, not a new screen — retailer scans the product's existing auto-generated SKU (as a QR/barcode tag) via `expo-camera` (already installed, no new dependency), app resolves SKU → product via a small addition to the existing products list endpoint, opens the existing `product/[id].tsx`, retailer taps the existing SOLD toggle. Full POS/RFID/AI-photo-diff-of-the-rack all researched and rejected as disproportionate or unreliable for this ICP — see `docs/PRO-REQUIREMENTS.md` §15 for the full comparison. GST-invoice-on-offline-sale flagged as a natural future hook, deliberately out of scope for this entry. **Shop staff (not just the owner) get this too, by default** — the status-update route has no owner-only gate, unlike the trash routes (see F-026 below), so scan-to-sell inherits staff access with no new permission code. **Do not start coding without explicit go-ahead.**

### F-026 BUG: Mobile "Recently Deleted" → permanent delete throws APIError — 🔴 Root-caused 2026-08-04, NOT FIXED
`apps/api/src/routes/products.ts` purge route calls `prisma.product.delete()` directly; F-017's DB guardrail trigger (`037_db_guardrails` migration, shipped 2026-07-26) blocks all hard deletes on `products` unless `SET app.allow_hard_delete = 'true'` is set first, which this route never does — the trigger's exception isn't the `P2003` code the route's catch block checks for, so it falls through as an unhandled 500 the mobile app shows as `APIError`. F-017 is the regression; the bypass pattern already works in `apps/api/src/jobs/purge-soft-deleted.ts` and just needs porting to this route inside a transaction. Full root-cause writeup: `docs/PRO-REQUIREMENTS.md` §16.
