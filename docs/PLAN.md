...
**BUILT (backend + first UI) 2026-08-17:** India Retailer Growth Engine — backend for the growth roadmap (`docs/INDIA-RETAILER-GROWTH.md`): 8 modules under `/v1/growth/*` (festival/reactivation/A-B campaigns, promotions, referrals, suppliers, showroom bookings, inventory alerts, product videos, AI translate — **khata + udhar removed from scope same day**) + public referral/booking endpoints + migration `055_growth_engine` (11 tables), all gated behind the `GROWTH_ENGINE` plan feature. Same day: mobile growth hub + full campaigns UI (list/create/edit/detail/send) + admin festival calendar CRUD + `/admin/festivals` page. **Remaining 7 modules UI pending; migration 055 not yet applied.** Spec: `docs/PRO-REQUIREMENTS.md` §27, build table: BUILD-LOG §44–45.
