...
### Future: F-031 Social Media Publishing — 🔴 Planned, DO NOT START
Spec written 2026-08-13 in `docs/PRO-REQUIREMENTS.md` §23 (user asked for the roadmap/discussion; no code). Retailer OAuth-connects their Instagram/Facebook in the app and pushes selected products or the collection link (`/c/[slug]`) to their social accounts. Facebook-first (single-product post → carousel → collection-link post), then Instagram Business (carousels, auto-captions from existing product data, per-platform CTA because Instagram captions can't carry clickable links), then automation (multi-platform one-tap post, opt-in auto-post on new arrivals mirroring F-022, UTM attribution). **Architecture decision recorded in spec:** build a generic connected-publishing-accounts module shared with F-022 (Google Business Profile) rather than a third parallel integration. **External blocker:** Meta app review for `pages_manage_posts` / `instagram_business_content_publish` (request well before dev). Not MVP scope — post-launch Phase 1 slot.
