...
- **Commit:** found 2026-09-22 while auditing `scripts/setup-role-separation.sql` against the schema (RC-029); no fix commit yet — recorded as an open gap
...
- **Fix:** for each of the 7, add the child sweep to **both** jobs (`purgeChildren(…)` in the cron, `DELETE FROM … WHERE retailer_id = $1` in the hard-delete), matching `product_videos` — the one bare-`retailer_id` table that already gets this treatment, and therefore the shape to copy. The five currently un-granted tables also need a `GRANT DELETE` (this file's list, or a migration — `promotions` got its own, migration `110`). `product_videos` proves the pattern works; the gap is that it was never generalised.
- **Proof:** the gap is mechanically enumerable, and was: parse `schema.prisma` for models declaring `retailer_id` **without** a `references: [id]` relation to `Retailer`, then diff against every table the purge consumers name. 13 candidates, 6 purged, **7 orphaned**. The check is the test that should be added — completeness, not just presence.
- **Prevention lesson:** for a denormalised `retailer_id`, prefer making cleanup a **schema-driven** obligation rather than a copy-pasted line — a test that fails when a bare `retailer_id` model is absent from both jobs turns a silent retention bug into a red build, which is the only reason the RESTRICT-FK tables were caught at all. Any new table with `retailer_id` and no FK should be treated as a purge-list change in the same PR, the way a RESTRICT FK already is.
