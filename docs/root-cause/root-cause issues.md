...
- **Component:** `packages/db/prisma/schema.prisma` (7 models) · `apps/api/src/jobs/purge-soft-deleted.ts` (cron) · `apps/api/src/jobs/purge-retailer-now.ts` (admin hard-delete) · `scripts/setup-role-separation.sql`
...
- **Fix:** all 7 now get the child sweep in **both** jobs (`purgeChildren(…)` in the cron, `DELETE FROM … WHERE retailer_id = $1` in the hard-delete) plus a `GRANT DELETE` — 6 in `scripts/setup-role-separation.sql`, `promotions` in migration `110`. `product_videos` proved the pattern works; the gap was that it was never generalised. **⚠ The four `customer_*`/`consent_*` tables are not yet a working fix** and the code says so: they have ROW LEVEL SECURITY enabled (migrations 079/100, no policies = default deny) and `kanchuki_purge` is created **without** `BYPASSRLS`, while no pre-existing purge target has RLS. RLS *filters* rather than raises, so those four may sweep 0 rows silently — strictly no worse than the previous state (nothing deleted them at all), but settling it needs a policy for the backend role or a role attribute, which is a PII-table decision, not a mechanical one. `campaign_sends`/`campaigns`/`promotions` have no RLS and work like `product_videos`.
