...
### Mid-cycle Plan Switching (Starter ↔ Growth ↔ Pro)
...
## 23. F-031 Social Media Publishing — Retailer connects Instagram/Facebook + posts products/collection links

**Status:** ✅ **Built (Phase 1 — Facebook Page, 2026-08-13).** `SocialAccount`/`SocialPost` + migration `052_social_publishing`, Meta Graph client (`apps/api/src/lib/meta-graph.ts`), connect/callback/publish/history/disconnect routes (`retailers-social.ts`), web `/social/connect` OTP flow + Page picker, mobile Settings → Social Media composer. Commits `fbe131a`/`efe6d63`/`148ef4a`; full build table `docs/BUILD-LOG.md` §41. **Remaining for Phase 1 go-live (Meta dashboard, user):** register `https://kanchuki.app/social/connect/callback` as Valid OAuth Redirect URI, add test account as Admin/Developer/Tester, and submit App Review for `pages_manage_posts`/`pages_show_list` before real retailers connect. **Phase 2 (Instagram Business — not started):** carousels, auto-captions, per-platform CTA (Instagram captions can't carry clickable links).
...
## 27. India Retailer Growth Engine — ✅ BUILT (backend + first UI) 2026-08-17
...
Migration `055_growth_engine` (11 tables — the `GROWTH_ENGINE` enum value
and its `plan_features` rows were split into migrations `056`/`057` because
PostgreSQL 55P04 forbids using a newly-added enum value in the same
transaction that adds it): `festivals`, `campaigns`
...
`suppliers` + `supplier_transactions`, `bookings`
(REQUESTED/CONFIRMED/CANCELLED/COMPLETED), `referrals` +
`referral_credits` (PENDING/CREDITED), `product_videos`. Existing-table
additions:
...
UI (list with type filters, create/edit form with festival picker,
placeholder chips, A/B editor, audience builder; detail screen with
live audience-count preview, send → WhatsApp API or wa.me manual links,
open-rate stats, delete) + admin festival calendar (`/v1/admin/festivals`
...
> Merged from the former `docs/customers/shopper-passport-identity.md`.
...
## 37. Market Research Foundation

> Merged from the former `docs/references/research/` folder. Original founding research now condensed here — the platform has narrowed scope significantly since (Manufacturer/Wholesaler layer, Fashion DNA matching, and checkout all removed; see §1 and the teardown row in BUILD-LOG.md).

### Final Research Report: AI Fashion Commerce Platform for Indian Retailers

> Merged from the former `docs/references/research/final-research.md`.
...
### Original Founding Blueprint (2026-07, superseded)

> Condensed from the former `docs/references/research/ai-fashion-sales-assistant-phase-1.md` (1,135 lines) — the earliest product vision, kept here for historical rationale rather than in full, since most of it describes scope that was never built or was later cut.

The original blueprint envisioned a 4-tier network — Manufacturer → Wholesaler → Retailer → Customer — with a 5-phase rollout (Phase 1 Retailer In-Store MVP → Phase 2 Wholesaler Catalog Network → Phase 3 Manufacturer Layer → Phase 4 Marketplace/Ordering → Phase 5 Advanced AI Platform), per-tier subscription monetization, and a Fashion DNA preference-matching engine as a core differentiator.

What actually shipped diverged early and stayed diverged: development committed to Phase 1 only (Retailer + Customer, no Wholesaler/Manufacturer tier ever built — see §2 Target Users and §10 Out of Scope), and Fashion DNA matching was built then fully removed 2026-08-31 (migration 082 teardown, BUILD-LOG.md). The AI auto-tagging, color-variant, and in-store search ideas from this blueprint did ship, in simplified form, as part of the actual MVP (§3).
