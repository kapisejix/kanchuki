...
> **RESOLVED 2026-09-07 (owner-approved cleanup):** the broken `prisma migrate deploy` step was removed from the `deploy-api` job — GH-runner egress to the Supabase pooler stays blocked (Supabase Network Restrictions), and the step ran as the app role which has no DDL grants, so it could never have applied migrations even if reachable. Prod migrations continue through the admin dashboard / Supabase SQL Editor with the migrator role, per SECURITY.md. Removing the step also un-blocked this workflow's own `railway up` deploy (the step was failing before it could run). See BUILD-LOG §2026-09-07 cleanup entry.
