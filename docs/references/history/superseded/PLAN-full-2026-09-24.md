...
> reference them are historical. Authoritative list: `docs/references/history/reports/2026-08-31-feature-teardown-spec.md`.
...
Collection-level share (`CollectionView.tsx`) already used the Web Share API; `ProductDetailSheet.tsx` had no share button until now. See `docs/PRO-REQUIREMENTS.md` F-006, `docs/references/research/feature-ideas-2026-07-30.md` §3.

### Future: F-021 Product & Store Ratings — 🔴 Planned
Reviewed 2026-07-30 (`docs/references/research/feature-ideas-2026-07-30.md` §2, spec in `docs/PRO-REQUIREMENTS.md` §10.12). Not in locked MVP scope — candidate for early Phase 1, after Phase 0 retention/conversion metrics are validated and there's repeat customer traffic worth rating. Gate rating eligibility behind a prior enquiry/order to prevent fake reviews. Includes `google_place_id` Google-review-link CTA (flagged review-gating risk documented in spec).
...
Spec `docs/PRO-REQUIREMENTS.md` §30 (supersedes F-032 Phase B); single spec `docs/tasks/pending/ai-photo-generation.md` §7 (both old task files are merged and deleted); full table `docs/BUILD-LOG.md` §2026-09-03. **Built (admin-only):** `fal-video.ts` lib (generateImageToVideo + 5 Fal models + ffmpeg crop/trim, 3/3 self-check), sync `POST /admin/photo-cleanup/image-to-video` bench route, `docs/ai-studio/AI Motion Styles.html` (16 motion presets), "AI Promo Video" card on `/admin/photo-cleanup-test`, and DB-driven admin addon-pack management (migration `089_resource_packs` applied; `/admin/resource-packs` screen + CRUD). **Owner decision 2026-09-03: retailer phase hard-deferred** — studio_styles `kind` (090) + quota seeds (091), queue/job, mobile modal, FB/IG publish all wait for bench sign-off. No F-034 mobile code exists.

### F-035 Kanchuki-Managed WhatsApp Sending (Meta Tech Provider + Embedded Signup) — 🔴 Planned, post-launch
Spec `docs/tasks/pending/whatsapp-managed-sending.md`; owner decision 2026-09-08 = **Model A** (each retailer gets their own WABA via an in-app Embedded Signup popup; Kanchuki provisions it and sends on their behalf — a single shared Kanchuki number for all retailers is rejected on Meta policy + branding grounds). The bulk-send machinery already exists (`POST /v1/collections/:id/bulk-send`, campaign-send, both reading `retailer.whatsapp_api_*`); F-035 replaces the manual credential-paste form with a one-tap connect flow. Long pole is Meta Business Verification + `whatsapp_business_messaging`/`whatsapp_business_management` App Review (4–8 weeks calendar). ~2–3 dev-weeks of code once approvals land: shared `lib/whatsapp-cloud.ts`, `retailers-whatsapp-embedded.ts` routes, `apps/web` Embedded Signup page, `webhooks/whatsapp.ts`, token encryption at rest, `whatsapp_template` table + migration, mobile settings rewrite, admin monitor. Fits **Phase II / Phase 3 (WhatsApp automation)** in the roadmap.

### F-036 Customer PWA — Home-Screen Icon, Visited-Store List & Push Notifications — 🟨 Phase A ✅ Built (2026-09-17); Phases B–D 🔴 Planned
Spec `docs/tasks/pending/customer-pwa-push-notifications.md`, full requirement `docs/PRO-REQUIREMENTS.md` §32, detail `docs/BUILD-LOG.md` §2026-09-17. Builds on the Shopper Passport identity work (`docs/customers/shopper-passport-identity.md`; `CustomerAccount`/`CustomerStoreVisit` already in schema via migrations `079`–`081`).

**Phase A ✅ Built:** an installable PWA icon that opens a `/my-stores` list of every retailer the customer has visited (tap-through to the existing catalog, changed nowhere), plus an install CTA at the two "just verified a visit" moments. Delivered: `apps/web/src/app/(shopper)/my-stores/{page.tsx,lib.ts}`, `public_slug` added to the passport-stores API select, `manifest.json` `start_url` → `/my-stores`, `lib/install-prompt.ts` + `components/InstallPrompt.tsx` (captured at module scope from the root layout — an effect-level listener provably missed the once-per-page-load event behind the two fetch-gated mount points). Verified: web tsc + 178/178, API tsc + 967/967, `[store]`/`ContactGate` untouched, zero `apps/mobile` files.

**Phases B–D 🔴 Planned:** B — Web Push on new-product upload and new-collection publish, a free additive channel wired to the existing product-create/collection-publish events (`PushSubscription` model + RLS, subscribe/unsubscribe API, VAPID + `web-push` send job, `push` handler in the currently precache-only service worker). **WhatsApp share is untouched** — push does not replace it, it only covers routine updates retailers wouldn't otherwise send manually. C — enforced iOS "Add to Home Screen" ahead of the permission prompt (Safari 16.4+ requires the PWA be installed before it can receive push at all — a plain tab cannot). D — consent/mute UI on `/my-profile` + aggregate subscriber counts for retailers. No dependency on Phase 1 AI Core; Phase B can start any time. **`return_to` is now consumed** (built later the same day): the guard sends a visitor without a passport to a dedicated `/login` route carrying the page they wanted, and a successful OTP returns them there. The parameter is validated against open redirects at both ends of the round trip, and the live browser suite proves the anonymous → `/login` → OTP → back-on-the-requested-page round trip (`docs/tasks/done/return-to-post-login-redirect.md` §8).

### F-037 Customer Engagement Enhancements + Admin Behavior Analytics — 🟨 Phase 1 ✅ Built (2026-09-18); Phases 2–4 🔴 Planned
Spec `docs/tasks/pending/customer-engagement-analytics.md`, full requirement `docs/PRO-REQUIREMENTS.md` §33, build detail `docs/BUILD-LOG.md` §2026-09-18. Owner follow-up to F-036. **Load-bearing schema correction:** the passport doc (`customer-qr-identity-solution.md` §15.1) claims `CustomerInteraction`/`CustomerFashionDNA` already exist and only need widening — they don't. Migration `082_remove_unwanted_features` (2026-08-31, one day after that doc was written) dropped both plus `store_affinities`. Only the passport identity core (`CustomerAccount`, `CustomerStoreVisit`, `ConsentEvent`, `PassportSession`, `CustomerRecentlyViewed`, `CustomerWishlistItem`) survived and is live — behavioral tracking (dwell time, search log, view history, "most liked") is net-new work, built directly at `CustomerAccount` scope, not a column-add. Scope: (1) engagement ideas not gated on new tracking — recently-viewed carousel surfaced, AI Stylist promoted, "complete the look" cross-sell, ratings (F-021), size-match filter front-and-center, perf/prefetch; (2) a net-new `CustomerInteraction` event log (view w/ real dwell_ms, search w/ query+filters+result_count, favorite, enquiry, store_visit) + nightly aggregation job (never query raw rows live) + admin dashboard (store-level aggregate default, `AuditLog`-gated per-customer drill-down) + retailer-facing aggregate view. Reuses the passport doc's `StoreAffinity` precompute pattern and existing Campaign Analytics/Commission Tracker admin UI conventions. Same DPDP profiling consent umbrella as personalized recs (§18) — no new consent flow. Explicitly not re-adding 360°/VTO/loyalty-points for engagement (all deliberately removed in the teardown). Roadmap: Phase 1 (event capture) → 2 (aggregation) → 3 (admin dashboard) → 4 (retailer view + data-dependent engagement items).

### Partner Network Manager (Marketing & Sales Enablement) — 🔴 Not built, schema currently broken
Spec `docs/PRO-REQUIREMENTS.md` §29, gap found by `docs/references/history/reports/2026-08-20-marketing-wiring-audit.md`. API route already exists (`apps/api/src/routes/retailers/retailers-partners/`) but `schema.prisma`'s `Partner`/`PartnerReferral` models don't compile (`npx prisma validate` fails — invalid inline enum syntax), no migration was ever applied, and `PARTNER_NETWORK` isn't in the `PlanFeatureKey` enum. Fix order: schema fix → split migrations (enum value, tables, plan-feature rows) → mobile screen (`apps/mobile/app/growth/partners.tsx`). No new admin page needed — existing plan-features grid handles gating once the enum value exists.
...
> This section = the admin-facing deploy dashboard/approval UI (below). For actual server hosting choices (Railway vs alternatives) and mobile app store launch steps, see `docs/references/guides/hosting-and-app-store.md`.
...
**Status:** ✅ **Completed** — F-013 through F-017 built, tested, and committed. See `docs/references/history/sessions/PROGRESS.md` 2026-07-26 for full details.
...
**Hosting cost/provider comparison + mobile store fees:** `docs/references/guides/hosting-and-app-store.md`
