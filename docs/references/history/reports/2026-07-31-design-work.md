...
**Cross-checked 2026-07-31 (same day, follow-up pass) — two items closed:**
- `accessibilityState` on selection chips: fixed. All 16 chip instances (`customer/[id].tsx` ×3, `product/[id].tsx` ×8, `product/add.tsx` ×3, `category/new.tsx` ×1, `product/catalog-import.tsx` ×1) now carry `accessibilityRole="button"` + `accessibilityState={{ selected }}` so screen readers announce toggle state, not just the visible checkmark.
- Mobile/web accent-color drift (rust/turmeric/sand): **was already fixed, doc was stale.** `apps/mobile/tailwind.config.js`'s hex values were byte-verified against `apps/web/tailwind.config.ts`'s current oklch tokens (converted via the CSS Color 4 OKLCH→sRGB formula) — they match exactly. The Red Elegance repaint (2026-07-29) had already synced mobile; this audit note just hadn't been corrected after.

**2026-07-31 (second follow-up pass) — tab bar, adaptivity, visual-polish primitives. User explicitly said no dark mode — light-only, gradient/shadow/animation direction instead:**
- **Tab bar 6→5:** `app/(tabs)/analytics.tsx` moved to `app/analytics.tsx` (top-level stack route, out of the tab group). Reachable via a new header icon button on Home (`BarChart3`, next to the existing StatCard taps that already routed there). `(tabs)/_layout.tsx` now registers 5 tabs, at spec.
- **Tablet/window adaptivity:** `src/hooks/useIsTablet.ts` (`useWindowDimensions`, 768px breakpoint) + `useGridColumns()` (2 phone / 3 tablet). Wired into all 5 `FlatList` product/category grids: `(tabs)/catalog.tsx`, `collection/new.tsx`, `category/index.tsx`, `category/[id].tsx`, `category/[id]/add-products.tsx` (each needs `key={columns}` — RN FlatList requires a remount on `numColumns` change). Not wired into every one of the 48 screens — single-column detail/list screens (orders, customers list, settings) don't need it.
- **Gradient/shadow/animation primitives (light-mode only, no `useColorScheme`):** `expo-linear-gradient` installed. New `src/components/AnimatedPressable.tsx` (Reanimated press-scale, 0.96 spring, skipped under Reduce Motion via the existing `useReduceMotion` hook) and `src/components/GradientButton.tsx` (gradient fill + iOS/Android shadow + press-scale) — the shared primitives, not a per-screen redo. `src/components/ProductCard.tsx` (the one shared card used by 6 screens' grids) got a real iOS shadow added — previously it only had Android `elevation`, iOS cards had zero shadow — plus swapped its `TouchableOpacity` for `AnimatedPressable`, so every product grid in the app got shadow + press animation for free. `GradientButton` was applied to the 3 highest-traffic primary CTAs: `product/add.tsx` (Save), `customer/add.tsx` (Save), `collection/new.tsx` (Create & Share) — not every button on every screen; the rest of the app's buttons are still plain `TouchableOpacity` and can adopt `GradientButton`/`AnimatedPressable` incrementally.
- **Honest scope note:** this is primitives + highest-traffic surfaces, not a hand-redesign of all 48 screens — there's no RN simulator/screenshot tool in this environment to visually verify a full sweep, so the safer lever (fix shared components once, apply to a bounded set of CTAs) was used instead of blind-editing every screen's markup.
