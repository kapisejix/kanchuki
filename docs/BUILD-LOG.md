...
**Verified:** API `tsc --noEmit` clean + `vitest run` **601/601** (63 new: 24 catalog-sync incl. 4 daily-cron + 3 timeout + 11 retailer routes + 12 webhook + 10 admin incl. cron-health + 6 auto-sync helpers); mobile `tsc --noEmit` clean + **43/43** (5 new); web `tsc --noEmit` clean. **Still pending:** migration `060` not applied (Supabase SQL Editor / `prisma migrate deploy`); no deployment; Meta webhook needs the dashboard setup in `docs/DEPLOY.md` §"Deploy WhatsApp Native Catalog Sync".
