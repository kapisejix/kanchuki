...
| AI try-on quality will be acceptable for ethnic wear (saree draping, unstitched suits) | HIGH | Test existing APIs (Replicate, FASHN, IDM-VTON) on ethnic wear samples |
...
1. **AI try-on quality for ethnic wear** — Saree draping, dupatta placement, and unstitched suit layering are technically harder than Western wear. Existing APIs (IDM-VTON, FASHN, Replicate) may produce poor results for Indian garments. You may need to fine-tune a custom model, which is expensive.
