    ...
    <>
      ...
      <Section>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-carbon/70 text-sm sm:text-base leading-relaxed space-y-3">
            <p><strong className="text-carbon">Kanchuki</strong> (कांचुकी / kanchuki) is the tailored bodice worn under a saree or ghagra — the quiet piece that makes everything else fit properly.</p>
      ...
      <Section className="bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-6">
            {BELIEFS.map((b) => (
              <div key={b.title} className="border-l-4 border-volt pl-5">
                <h3 className="font-display text-base font-semibold text-carbon mb-1">{b.title}</h3>
                <p className="text-sm text-carbon/60 leading-relaxed">{b.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <Section>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <ul className="space-y-3">
            {BUILT.map((b) => (
              <li key={b} className="flex gap-3 text-sm sm:text-base text-carbon/70 leading-relaxed">
                <span className="text-cobalt-600 mt-1">✓</span>
                <span>{b}</span>
              </li>
            ))}
          </ul>
          <p className="mt-6 text-sm text-carbon/40">Full feature list with honest status: <a href="/for-retailers" className="text-cobalt-600 font-medium hover:underline">/for-retailers</a>.</p>
        </div>
      </Section>
      ...
      <Section className="bg-white">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="rounded-2xl border border-dashed border-carbon/25 bg-white p-6 sm:p-8">
            <p className="text-sm sm:text-base text-carbon/60 leading-relaxed italic">
