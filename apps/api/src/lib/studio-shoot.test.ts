...
/**
 * Explicit timeout for the pipeline tests below. They are slow BY
 * CONSTRUCTION, not by accident: each mocked generation drives the real poll
 * loop, so every extra mocked poll response costs a real production
 * `POLL_INTERVALS_MS` sleep of 1_000 ms (`studio-shoot.ts`) — the mocked
 * `Processing` → `Ready` test measures ~1.0 s for exactly that reason. A
 * two-step pipeline (reference → try-on → scene) measures ~3.0 s and the A/B
 * order comparisons ~4.5 s, leaving the slowest tests **443 ms** of headroom
 * against vitest's 5 s default.
 *
 * That margin is why they failed under contention: with two suites running
 * concurrently the same tests reported `Test timed out in 5000ms` while the
 * assertion and the code beneath it were both correct. A slower CI machine
 * reaches the same point with no contention at all. 30 s is ~6.5× the slowest
 * measurement taken here.
 *
 * A real hang still fails: the poll loop's own deadline is 180 s
 * (`POLL_TIMEOUT_MS`), so this constant only stops a loaded box from reading as
 * a code bug. The durable fix is to make the poll interval injectable so these
 * tests need no real sleep — flagged, not done here (docs/BUILD-LOG.md
 * §2026-09-24, later).
 */
...
describe('generateStudioImage', () => {
  it(
    'submits with x-key + prompt + input URL, polls to Ready, returns sample',
  ...
  it(
    'vton_kontext: generates a reference, tries the garment on, then swaps the scene',
    { timeout: PIPELINE_TEST_TIMEOUT_MS },
    async () => {
  ...
  it(
    'vton_kontext: uses a supplied model reference and skips reference generation',
    { timeout: PIPELINE_TEST_TIMEOUT_MS },
    async () => {
      vi.stubEnv('FAL_API_KEY', 'fal-key');
  ...
  it(
    'vton_kontext: falls back to single-shot Kontext when try-on fails',
    { timeout: PIPELINE_TEST_TIMEOUT_MS },
    async () => {
      vi.stubEnv('FAL_API_KEY', 'fal-key');
  ...
  it(
    'runs both orders and feeds each try-on what that order implies',
    { timeout: PIPELINE_TEST_TIMEOUT_MS },
    async () => {
      vi.stubEnv('FAL_API_KEY', 'fal-key');
      ...
      expect(kontext.filter((b) => b.image_url === 'https://r2.example/product.jpg')).toHaveLength(
        1,
      );
  ...
  it(
    'fails one arm without masking it or losing the other',
    { timeout: PIPELINE_TEST_TIMEOUT_MS },
    async () => {
      vi.stubEnv('FAL_API_KEY', 'fal-key');
      mockFetch.mockReset();
  ...
  it(
    'reversed + Gemini: re-serves the base64 intermediate before the try-on',
    { timeout: PIPELINE_TEST_TIMEOUT_MS },
    async () => {
      vi.stubEnv('FAL_API_KEY', 'fal-key');
      ...
      const [forward, reversed] = ab.arms;
      expect(reversed?.stages.map((s) => s.url)).toEqual([
        'https://r2.example/persisted-scene.jpg',
      ]);
  ...
  it(
    'reversed + Gemini without a persist step: fails with the reason, no try-on',
    { timeout: PIPELINE_TEST_TIMEOUT_MS },
    async () => {
      vi.stubEnv('FAL_API_KEY', 'fal-key');
      vi.stubEnv('GEMINI_API_KEY', 'gemini-key');
  ...
  it(
    'gpt_image_2_medium: sends the photo, prompt and quality to the GPT Image 2 edit endpoint',
    { timeout: PIPELINE_TEST_TIMEOUT_MS },
    async () => {
      vi.stubEnv('FAL_API_KEY', 'fal-key');
  ...
  it(
    'vton_gemini: try-on first, then Gemini renders the scene',
    { timeout: PIPELINE_TEST_TIMEOUT_MS },
    async () => {
      vi.stubEnv('FAL_API_KEY', 'fal-key');
      vi.stubEnv('GEMINI_API_KEY', 'gemini-key');
  ...
  it(
    'vton_gemini: falls back to single-shot Kontext when try-on fails',
    { timeout: PIPELINE_TEST_TIMEOUT_MS },
    async () => {
      vi.stubEnv('FAL_API_KEY', 'fal-key');
      vi.stubEnv('GEMINI_API_KEY', 'gemini-key');
