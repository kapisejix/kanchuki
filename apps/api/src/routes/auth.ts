...
import { validationError, AppError } from '../plugins/error-handler.js'
...
export const authRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.post('/otp/send', async (request, reply) => {
    ...
    if (!body.success) throw validationError(body.error.issues[0]?.message ?? 'Invalid phone', 'phone')
    ...
    if (error) {
      ...
      throw new AppError('OTP_SEND_FAILED', 'Failed to send OTP. Check phone number and try again.', 400)
    }
