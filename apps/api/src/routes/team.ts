...
import { prisma, type TeamRole } from '@kanchuki/db'
import { normalizeIndianPhone } from '@kanchuki/shared'
import { forbidden, notFound, validationError } from '../plugins/error-handler.js'
import { hashPassword, verifyPassword, signTeamToken, verifyTeamToken } from '../plugins/team-auth.js'
...
const OnboardRetailerSchema = z.object({
  phone: z.string().min(10).max(15).transform((v) => normalizeIndianPhone(v)),
...
export const teamRoutes: FastifyPluginAsync = async (server) => {
  server.addHook('preHandler', async (request, reply) => {
    ...
    if (validAdminKey(adminKey)) {
      request.teamMember = { id: 'admin-key', role: 'SUPER_ADMIN', territoryIds: [], isSuperAdmin: true }
      return
    }

    const authHeader = request.headers.authorization
    if (!authHeader?.startsWith('Bearer ')) {
      return reply.status(401).send({ error: { code: 'UNAUTHORIZED', message: 'Missing Bearer token', status: 401 } })
    }

    const claims = await verifyTeamToken(authHeader.slice(7))
    if (!claims) {
      return reply.status(401).send({ error: { code: 'UNAUTHORIZED', message: 'Invalid or expired token', status: 401 } })
    }
    ...
    if (!member || !member.is_active) {
      return reply.status(403).send({ error: { code: 'FORBIDDEN', message: 'Account inactive or removed', status: 403 } })
    }
  ...
  server.post('/login', async (request) => {
    ...
    if (!body.success) throw validationError(body.error.issues[0]?.message ?? 'Invalid credentials')

    const member = await prisma.teamMember.findUnique({ where: { email: body.data.email.toLowerCase() } })
  ...
  server.post('/members', async (request) => {
    ...
    const existing = await prisma.teamMember.findUnique({ where: { email: body.data.email.toLowerCase() } })
    ...
    const member = await prisma.teamMember.create({
      ...
      select: { id: true, name: true, email: true, role: true, max_retailers: true, is_active: true },
    })
    return { data: member }
  })
  ...
  server.patch<{ Params: { id: string } }>('/members/:id', async (request) => {
    ...
    const member = await prisma.teamMember
      .update({
        ...
        select: { id: true, name: true, email: true, role: true, is_active: true, max_retailers: true },
      })
      .catch(() => null)
    if (!member) throw notFound('Team member')
    return { data: member }
  ...
  server.post('/retailers', async (request) => {
    ...
    const territoryId = body.data.territory_id ?? (await deriveTerritoryFromPincode(body.data.pincode))
    ...
    if (tm.id !== 'admin-key') {
      const member = await prisma.teamMember.findUnique({ where: { id: tm.id }, select: { max_retailers: true } })
      if (member?.max_retailers != null) {
        const count = await prisma.retailer.count({ where: { onboarded_by_id: tm.id, deleted_at: null } })
  ...
  server.get('/retailers', async (request) => {
    const tm = request.teamMember!
    const where = tm.isSuperAdmin ? { deleted_at: null } : { deleted_at: null, territory_id: { in: tm.territoryIds } }
