...
import { type AbVariantResult, abTestSignificance, accumulator } from '../growth-helpers.js';
import { requireGrowth } from './growth-campaigns-helpers.js';
// Bucket factories — passed to `accumulator` so the object literal is only
// allocated for a genuinely new key.
const emptyCampaignBucket = () => ({ sent: 0, opened: 0, campaigns: 0 });
const emptySentBucket = () => ({ sent: 0, opened: 0 });
const emptyViewBucket = () => ({ views: 0, enquiries: 0 });
...
export const growthCampaignAnalyticsRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.get('/campaigns/stats', async (request) => {
    ...
    for (const c of campaigns) {
      ...
      const typeBucket = accumulator(by_type, c.type, emptyCampaignBucket);
      typeBucket.sent += c.sent_count;
      typeBucket.opened += c.opened_count;
      typeBucket.campaigns += 1;
      const key = c.festival_name ?? 'Other';
      const festivalBucket = accumulator(by_festival, key, emptyCampaignBucket);
      festivalBucket.sent += c.sent_count;
      festivalBucket.opened += c.opened_count;
      festivalBucket.campaigns += 1;
    }
  ...
  server.get('/analytics', async (request) => {
      ...
      const segment = !c
        ...
        : (c.total_purchases ?? 0) === 0
          ...
          : (c.total_spent ?? 0) >= 200_000
            ? 'VIP'
            : 'REGULAR';
      const segmentBucket = accumulator(by_segment, segment, emptySentBucket);
      segmentBucket.sent += 1;
      if (row.opened_at) {
        segmentBucket.opened += 1;
        openedTotal += 1;
    ...
    // accumulator below stays typed while by_category / video_vs_photo keep
    // serialising (the mobile analytics screen reads them).
    type InteractionRow = {
      type: string;
      product?: { category?: string | null; videos: unknown[] } | null;
    };
    ...
    for (const i of interactions) {
      ...
      const categoryBucket = accumulator(by_category, category, emptyViewBucket);
      if (i.type === 'view') categoryBucket.views += 1;
      else categoryBucket.enquiries += 1;
    ...
    for (const c of campaigns) {
      ...
      by_variant.push({
        ...
        variants: results,
        significance: abTestSignificance(variantA, variantB),
      });
    }
