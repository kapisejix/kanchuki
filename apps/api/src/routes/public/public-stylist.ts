...
function matchToColor(product: { primary_color: string | null; secondary_colors: string[] }, targetColor: string): boolean {
  ...
  server.post(
    ...
    async (request, reply) => {
      ...
      if (fabricHints.length > 0) {
        const filtered = candidates.filter((p) => fabricHints.some((f) => p.fabric_estimate?.toLowerCase().includes(f)))
      ...
      const catalogForLLM = candidates.slice(0, 60).map((p) => ({
        ...
        price: p.price_min ? `₹${p.price_min}${p.price_max && p.price_max !== p.price_min ? `–₹${p.price_max}` : ''}` : 'Price on request',
...
- Return ONLY a valid JSON array, no markdown, no explanation`,
        ...
        const recommendations = jsonMatch ? JSON.parse(jsonMatch[0]) as Array<{
          product_id: string
          name: string
          category: string
          color: string
