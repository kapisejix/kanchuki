...
export default function InStoreTryOnScreen() {
  ...
  const handleRunTryOn = async () => {
    ...
    try {
      ...
      // Upload customer photo to R2 — uploadImageToR2 already throws if the
      // presigned PUT doesn't come back 2xx, which is the authoritative,
      // strongly-consistent signal from R2's S3 API. A follow-up HEAD against
      // the public pub-*.r2.dev CDN domain raced the CDN's cache/propagation
      // and produced false "incomplete" failures on real devices even when
      // the object landed fine — removed rather than relaxed further.
