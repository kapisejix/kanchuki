...
export default function CreateSocialPostScreen() {
  ...
  const profile = (
    profileData as { data?: { shop_name?: string | null; public_slug?: string | null } } | undefined
  )?.data;
  ...
  // COLLECTION_LINK sub-choice: 'link' posts one FB link card; 'carousel'
  ...
  const postTypeCtx = useCallback(
    (next: SocialPostComposeType) => {
      ...
      if (next === 'COLLECTION_LINK') {
        // Default to the link card — media-less, so IG (which can't post a
        ...
        setTargetIds((cur) => {
          ...
          return kept.length === 0
            ? accounts.filter((a) => a.platform === 'FACEBOOK').map((a) => a.id)
            : kept;
        });
      } else {
        if (next === 'CAROUSEL') {
  ...
  // Switching the collection format juggles IG targets: a photo carousel is
  // fine on Instagram, a bare link card is not.
  const changeCollectionFormat = useCallback(
    (fmt: 'link' | 'carousel') => {
      setCollectionFormat(fmt);
      if (fmt === 'carousel') {
        setTargetIds((cur) => (cur.length === 0 ? accounts.map((a) => a.id) : cur));
      } else {
        setTargetIds((cur) => {
          const ig = new Set(accounts.filter((a) => a.platform === 'INSTAGRAM').map((a) => a.id));
          const kept = cur.filter((id) => !ig.has(id));
  ...
  const targetDisabledReason = useCallback(
    (account: SocialAccountInfo) =>
      postType === 'COLLECTION_LINK' &&
      collectionFormat === 'link' &&
      account.platform === 'INSTAGRAM'
        ? "Instagram can't post a link without photos — switch Format to Photo carousel for Instagram."
        : null,
    [postType, collectionFormat],
  );
  ...
  const resolveMedia = useCallback(
    (productId: string): ComposeMedia | null => {
      ...
      const photo =
        detail.photos.find((p) => p.is_primary && !p.is_video) ??
        detail.photos.find((p) => !p.is_video);
  ...
  const problems = useMemo(() => {
    ...
    if (postType === 'COLLECTION_LINK') {
      if (!collectionLink) {
        ...
        if (collectionDetailQuery.isLoading) {
          list.push('Loading the collection…');
        } else if (collectionCarouselItems.length < 2) {
          list.push('This collection needs at least 2 products with photos for a carousel.');
        }
      }
    } else {
  ...
  const templateValues = useMemo(() => {
    ...
    const names = selected.map((p) => p.name?.trim()).filter((n): n is string => Boolean(n));
    const link =
  ...
  const payloadSignature = useMemo(() => {
    ...
    return [
      ...
      postType === 'COLLECTION_LINK' ? collectionFormat : '',
      postType === 'COLLECTION_LINK' && collectionFormat === 'carousel'
        ? collectionCarouselItems.map((i) => i.photo_id).join(',')
        : '',
  ...
  const buildPayload = useCallback(
    (clientPostId: string): CreateSocialPostInput => {
      if (postType === 'COLLECTION_LINK') {
        ...
        // collection URL to the caption.
        if (collectionFormat === 'carousel') {
          return {
      ...
      return {
        ...
        ...(linkType === 'collection' && linkCollection
          ...
          : {}),
        link_type: linkType,
        ...(linkType === 'product' && selected.length === 1
          ? { link_product_id: selected[0].id }
          : {}),
        caption: caption.trim() || undefined,
        template_id: templateId ?? undefined,
      };
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const results =
        (res as { data?: { results?: SocialPostTargetResult[] } }).data?.results ?? [];
    ...
    <View className="flex-1 bg-ink-50">
          ...
          >
            ...
            {postType === 'COLLECTION_LINK' ? (
              <>
                    ...
                    collections.map((c, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {active ? (
                              <View className="w-2.5 h-2.5 rounded-full bg-ink-600" />
                            ) : null}
                          </View>
                        </AnimatedPressable>
                      );
                ...
                {collectionLink ? (
                  <>
                    {sectionTitle('3', 'Format', 'How this collection posts')}
                    <View className="flex-row gap-2">
                      {[
                        {
                          value: 'link' as const,
                          label: 'Link card',
                          hint: 'One preview card — Facebook only',
                        },
                        {
                          value: 'carousel' as const,
                          label: 'Photo carousel',
                          hint: 'Swipeable product photos — FB + Instagram',
                        },
                      ].map((opt) => {
                        const active = collectionFormat === opt.value;
                        return (
                          <AnimatedPressable
                            key={opt.value}
                            onPress={() => changeCollectionFormat(opt.value)}
                            accessibilityRole="button"
                            accessibilityState={{ selected: active }}
                          ...
                          >
                            <Text
                              className={`text-xs font-bold ${active ? 'text-white' : 'text-sand-800'}`}
                            >
                              {opt.label}
                            </Text>
                            <Text
                              className={`text-[10px] mt-0.5 ${active ? 'text-white/70' : 'text-sand-400'}`}
                            >
                              {opt.hint}
                            </Text>
                          </AnimatedPressable>
                    ...
                    {collectionFormat === 'carousel' && !collectionDetailQuery.isLoading ? (
                      <Text className="text-[10px] text-sand-400 mt-1.5 px-1">
                        {collectionCarouselItems.length > 0
                          ? `${collectionCarouselItems.length} product ${
                              collectionCarouselItems.length === 1 ? 'photo' : 'photos'
                            } will be posted (max ${CAROUSEL_CAP}).`
              ...
              <>
                {sectionTitle(
                  ...
                  selected.length > 0
                    ? `${selected.length}/${postType === 'CAROUSEL' ? CAROUSEL_CAP : 1} picked`
                    : undefined,
                )}
                <ProductGridPicker
                ...
                {items.length > 0 ? (
                  <>
                    {sectionTitle(
                      '3',
                      'Media per product',
                      postType === 'CAROUSEL' ? 'Photos only for carousels' : 'Photo or video',
                    )}
                    <View className="gap-4">
                      {items.map(({ product, media }) => (
                        <View
                          key={product.id}
                          className="bg-white rounded-2xl border border-sand-100 p-4"
                        >
                          <ItemMediaStrip
                            product={product}
                            selection={media}
                            ...
                            onSelect={(m) =>
                              setMediaOverride((cur) => ({ ...cur, [product.id]: m }))
                            }
                          />
                        </View>
                      ))}
                      ...
                      <AnimatedPressable
                        ...
                        className={`rounded-full border px-3.5 py-2 ${
                          active
                            ...
                            : disabled
                              ? 'bg-sand-50 border-sand-100 opacity-50'
                ...
                {linkType === 'collection' ? (
                  <View className="bg-white rounded-2xl border border-sand-100 overflow-hidden">
                    {collections.length === 0 ? (
                      <Text className="text-xs text-sand-400 p-4 text-center">
                        No collections yet
                      </Text>
                    ) : (
                      collections.map((c, i) => {
                        ...
                        return (
                          ...
                          >
                            <Text
                              className="text-xs font-semibold text-sand-800 flex-1"
                              numberOfLines={1}
                            >
                              {c.title}
                            </Text>
            ...
            <Text className="text-[10px] text-sand-400 text-center mt-3 px-6 leading-4">
              Preview is approximate — Kanchuki builds the final post and link server-side before
              publishing.
            </Text>
          </ScrollView>
      ...
      {result ? (
        <ResultSheet
          ...
          onDone={() =>
            result.every((r) => r.status === 'POSTED') ? router.back() : setResult(null)
          }
        />
      ) : null}
    </View>
        ...
        >
          ...
          <ScrollView className="max-h-[45%]">
            {results.map((r) => {
              ...
              return (
                <View
                  key={r.social_account_id}
                  className="flex-row items-center bg-sand-50 rounded-2xl px-4 py-3 mb-2"
                >
                  <View
                  ...
                  <View className="flex-1">
                    ...
                    {r.status === 'POSTED' ? (
                      r.external_post_url ? (
                        <AnimatedPressable
                          onPress={() => void Linking.openURL(r.external_post_url!)}
                          hitSlop={6}
                          accessibilityRole="button"
                        >
                          <Text className="text-[11px] font-semibold text-ink-700 underline">
                            View post ↗
                          </Text>
                        </AnimatedPressable>
                      ) : (
